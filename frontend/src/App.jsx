// frontend/src/App.jsx - КРИТИЧЕСКИЕ ИСПРАВЛЕНИЯ
...
function PageWrapper({ children }) {
  ...
  return (
    <div
      className={`page-anim ${enter ? "enter" : ""}`}
      style={{
        opacity: enter ? 1 : 0,
...
export default function App() {
  ...
  const [city, setCity] = useState(null); // ИСПРАВЛЕНО: null вместо дефолтного города
  ...
  async function initializeApp() {
    try {
      ...
      // ИСПРАВЛЕНО: получаем данные пользователя из Telegram
      const tgUser = getTelegramUser();

      if (!tgUser) {
        console.warn('⚠️ Не удалось получить данные из Telegram');
        setInitError('Не удалось получить данные пользователя из Telegram');

        // ИСПРАВЛЕНО: Если нет Telegram данных, переходим к капче с гостевым режимом
      ...
      // ИСПРАВЛЕНО: НЕ восстанавливаем город автоматически
      try {
  ...
  async function handleCitySelected(selectedCity) {
    ...
    // Теперь обрабатываем пользователя если есть данные Telegram
    if (telegramUser) {
      try {
  ...
  if (appState === "city") {
    // ИСПРАВЛЕНО: НЕ передаем дефолтный город, пусть пользователь выбирает
    return <CitySelector onSelect={handleCitySelected} defaultCity={city || null} />;
  }
