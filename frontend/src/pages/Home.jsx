// frontend/src/pages/Home.jsx - ИСПРАВЛЕННАЯ ВЕРСИЯ БЕЗ БАГОВ
...
export default function Home({ city = "Москва", user }) {
  ...
  const handleSearch = useCallback(async (e) => {
    ...
    setLoading(true);
    try {
      const results = await storesAPI.searchProducts(searchQuery, { city, limit: 50 });
      setSearchResults({
        query: searchQuery,
        products: results || [],
  ...
  // ИСПРАВЛЕННЫЙ клик по категории - теперь показывает товары
  const handleCategoryClick = useCallback(async (category) => {
    setLoading(true);
    try {
      // Получаем товары категории
      const products = await storesAPI.searchProducts("", {
        city,
      ...
      alert(`Ошибка загрузки категории: ${error.message}`);
    } finally {
      setLoading(false);
    }
  }, [city]);
  ...
  return (
    <div className="home-page">
      {/* ИСПРАВЛЕННЫЙ Pull-to-refresh indicator - СТРОГО ПО ЦЕНТРУ */}
      {showIndicator && (
        <div
      ...
      {/* ИСПРАВЛЕННОЕ Search Modal - центровка через flex */}
            ...
            <div className="search-modal-header">
              ...
              <div className="search-info">
                <h2 className="search-title">
                  {searchResults.isCategory ? 'Категория' : 'Результаты поиска'}
                </h2>
                    ...
                    <div key={product.id} className="search-product-card" onClick={() => handleProductClick(product)}>
                      ...
                      <div className="product-info">
                        ...
                        <p className="product-description">
                          {product.short_description || 'Качественный товар'}
                        </p>
      ...
      {/* ПЕРЕИМЕНОВАННАЯ СЕКЦИЯ: Популярные категории */}
      ...
      <style jsx>{`
        ...
        /* ИСПРАВЛЕННОЕ центрирование pull-to-refresh */
        ...
        /* ИСПРАВЛЕННОЕ центрирование модальных окон */
