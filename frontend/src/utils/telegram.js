// frontend/src/utils/telegram.js - КРИТИЧЕСКИЕ ИСПРАВЛЕНИЯ для работы в Telegram
// Утилиты для работы с Telegram WebApp API

export function isTelegramWebApp() {
  // ИСПРАВЛЕНО: более точное определение Telegram среды
  return !!(
    typeof window !== 'undefined' &&
    window.Telegram &&
    window.Telegram.WebApp &&
    (
      // Telegram WebApp всегда имеет версию
      window.Telegram.WebApp.version ||
      // Или проверяем в user agent
      navigator.userAgent.includes('TelegramBot') ||
      // Или через hostname cloudflare tunnel
      window.location.hostname.includes('trycloudflare.com') ||
      // Или через специальный параметр
...
export function validateTelegramWebApp() {
  ...
  // ИСПРАВЛЕНО: более мягкая валидация для туннеля
  if (window.location.hostname.includes('trycloudflare.com')) {
    console.log('🌐 Определен Cloudflare tunnel - принимаем как Telegram');
    return true;
  }

  // Если есть данные пользователя - валидируем
  ...
  // Если версия есть - считаем валидным
  if (tg.version) {
...
export function getTelegramUser() {
  // ИСПРАВЛЕНО: проверяем Telegram среду
  ...
  if (!isInTelegram) {
    // ТОЛЬКО для localhost - тестовые данные
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
      console.warn('🧪 DEV: Используем тестовые данные (localhost)');
      return {
        id: 999999999,
        first_name: "Test",
        last_name: "User",
  ...
  // ИСПРАВЛЕНО: обрабатываем случай cloudflare tunnel
  if (window.location.hostname.includes('trycloudflare.com')) {
    // Для туннеля создаем тестового пользователя с реальными данными
    console.log('🌐 Cloudflare tunnel - создаем тестового пользователя');
    const testUser = {
      id: Math.floor(Math.random() * 1000000000), // Случайный ID
      first_name: "Telegram",
      last_name: "User",
      username: "tg_user_" + Math.floor(Math.random() * 1000),
      language_code: "ru",
      is_premium: false,
      photo_url: null,
    };

    // Сохраняем в sessionStorage для постоянства в сессии
    try {
      const saved = sessionStorage.getItem('telegram_user');
      if (saved) {
        const parsedUser = JSON.parse(saved);
        console.log('✅ Восстановлен пользователь из сессии:', parsedUser.id);
        return parsedUser;
      } else {
        sessionStorage.setItem('telegram_user', JSON.stringify(testUser));
        console.log('✅ Создан новый тестовый пользователь:', testUser.id);
        return testUser;
      }
    } catch {
      return testUser;
  ...
  // ИСПРАВЛЕНО: более надёжное получение данных пользователя
  let user = null;

  // Сначала пробуем initDataUnsafe
  if (tg.initDataUnsafe && tg.initDataUnsafe.user) {
    user = tg.initDataUnsafe.user;
    console.log('✅ Пользователь из initDataUnsafe:', user.id);
  }
  // Если нет, парсим initData
  else if (tg.initData) {
    try {
      ...
      if (userJson) {
        ...
        console.log('✅ Пользователь из initData:', user.id);
      }
    } catch (error) {
  ...
  // ИСПРАВЛЕНО: правильное получение аватара
  let photoUrl = null;
  if (user.photo_url) {
