# backend/models.py - ИСПРАВЛЕНО: добавлено order_id в BalanceRequest
...
# ИСПРАВЛЕНО: добавлен order_id в BalanceRequest
class BalanceRequest(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    order_id: str = Field(unique=True, index=True)  # ДОБАВЛЕНО: уникальный номер заявки
